...
devDQA = pickle.load(open('datacache/devDQA.p', 'rb'))
...
predict_fn = predictor.from_saved_model('savemodel/savemodelDQA')
...
def predict_examples(n):
    ...
    devYin = np.expand_dims(devDQA[n], axis=0)
...
def predict_dialog(devXin, pred_devND, dev_turns_in):
    ...
    devYin = np.expand_dims(devDQA[0], axis=0)
