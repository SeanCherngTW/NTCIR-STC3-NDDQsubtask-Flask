...
devDQE = pickle.load(open('datacache/devDQE.p', 'rb'))
...
predict_fn = predictor.from_saved_model('savemodel/savemodelDQE')
...
def predict_examples(n):
    ...
    devYin = np.expand_dims(devDQE[n], axis=0)
...
def predict_dialog(devXin, pred_devND, dev_turns_in):
    ...
    devYin = np.expand_dims(devDQE[0], axis=0)
